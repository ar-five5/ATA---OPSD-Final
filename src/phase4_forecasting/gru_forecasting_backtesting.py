...
df = pd.read_csv(os.path.join(base_dir, 'data', 'time_series_60min_singleindex.csv'), index_col=0, parse_dates=True)
...
for country in countries:
    ...
    checkpoint = torch.load(os.path.join(base_dir, f'results/phase3d_gru_results/{country}_gru_model.pt'), weights_only=False)
...
metrics_df.to_csv(os.path.join(base_dir, 'results/phase4c_gru_results/metrics_comparison.csv'))
print(f"✓ Saved metrics to results/phase4c_gru_results/metrics_comparison.csv")
...
for country in countries:
    ...
    result_df.to_csv(os.path.join(base_dir, f'results/phase4c_gru_results/forecast_data_{country}.csv'), index=False)
...
for idx, country in enumerate(countries):
    ...
    df_plot = pd.read_csv(os.path.join(base_dir, f'results/phase4c_gru_results/forecast_data_{country}.csv'), parse_dates=['timestamp'])
    df_plot = df_plot.head(240)  # First 10 days
    
    ax.plot(df_plot['timestamp'], df_plot['actual'], 'k-', label='Actual', linewidth=1.5)
    ax.plot(df_plot['timestamp'], df_plot['forecast'], 'b--', label='GRU Forecast', linewidth=1.5, alpha=0.7)
    
    ax.set_title(f'{country} - GRU 24-hour Forecast (MAPE: {all_metrics[country]["MAPE_%"]:.2f}%)', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig(os.path.join(base_dir, 'results/phase4c_gru_results/forecast_comparison.png'), dpi=300, bbox_inches='tight')
...
ax.set_title('GRU Model Performance Metrics by Country', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig(os.path.join(base_dir, 'results/phase4c_gru_results/metrics_bar_chart.png'), dpi=300, bbox_inches='tight')
